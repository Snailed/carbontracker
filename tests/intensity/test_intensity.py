...
class TestIntensity(unittest.TestCase):
    ...
    def test_get_default_intensity_location_failure(self, mock_geocoder_ip):
        ...
        self.assertEqual(
            result["carbon_intensity"], constants.WORLD_2019_CARBON_INTENSITY
        )
        self.assertIn("Defaulted to average carbon intensity", result["description"])
    ...
    def test_CarbonIntensity_set_as_default(self, mock_read_csv, mock_geocoder_ip):
        mock_location = MagicMock()
        mock_location.ok = True
        mock_location.address = "Sample Address"
        mock_location.country = "US"
        ...
        self.assertEqual(
            default_intensity["carbon_intensity"], constants.WORLD_2019_CARBON_INTENSITY
        )
    ...
    def test_carbon_intensity_location_failure(self, mock_geocoder_ip):
        ...
        with patch(
            "carbontracker.emissions.intensity.intensity.default_intensity",
            ...
            self.assertEqual(
                result.carbon_intensity, default_intensity["carbon_intensity"]
            )
            self.assertEqual(result.address, "UNDETECTED")
            self.assertEqual(result.success, False)
            self.assertIn(
                "Live carbon intensity could not be fetched at detected location",
                result.message,
            )
    ...
    def test_set_carbon_intensity_message(self, mock_geocoder_ip):
        ...
        fallback_address = (
            "Generic Location, Country"  # The fallback or generic location
        )
        ...
        def set_expected_message(is_prediction, success, carbon_intensity):
            if is_prediction:
                ...
                if success:
                    ...
                    message = (
                        f"Live carbon intensity could not be fetched at detected location: {detected_address}. "
        ...
        with patch(
            ...
            for is_prediction, success, carbon_intensity in scenarios:
                ...
                ci.success = success
                ci.carbon_intensity = (
                    carbon_intensity if carbon_intensity is not None else 0.0
                )
                intensity.set_carbon_intensity_message(
                    ci,
                    datetime.now(timezone.utc),
                    datetime.now(timezone.utc) + timedelta(seconds=time_dur),
                )
                expected_message = set_expected_message(
                    is_prediction, success, carbon_intensity
                )
                self.assertEqual(ci.message, expected_message)

    @patch("geocoder.ip")
    @patch(
        "carbontracker.emissions.intensity.fetchers.electricitymaps.ElectricityMap.suitable"
    )
    def test_carbon_intensity_address_assignment(
        self, mock_electricity_map_suitable, mock_geocoder_ip
    ):
        mock_location = MagicMock()
        mock_location.ok = True
    ...
    @patch("geocoder.ip")
    @patch(
        "carbontracker.emissions.intensity.fetchers.electricitymaps.ElectricityMap.carbon_intensity"
    )
    def test_carbon_intensity_failure(
        self, mock_electricity_map_carbon_intensity, mock_geocoder_ip
    ):
        mock_location = MagicMock()
        mock_location.ok = True
        mock_geocoder_ip.return_value = mock_location
    ...
    @patch(
        "carbontracker.emissions.intensity.fetchers.carbonintensitygb.CarbonIntensityGB"
    )
    @patch("carbontracker.emissions.intensity.intensity.geocoder.ip")
    def test_carbon_intensity_exception_carbonintensitygb(
        self, mock_geocoder, mock_carbonintensitygb
    ):
    ...
    @patch(
        "carbontracker.emissions.intensity.fetchers.energidataservice.EnergiDataService"
    )
